...
<body style="overflow-x: hidden">
	...
	<div class="wrapper">
					...
					<div class="row">
						<div class="hero-slider-content col-sm-8 col-xs-12">
							<h1 class="wow fadeInUp" data-wow-duration="1s"
								data-wow-delay="0.5s"><%=Products.get(k).getName()%></h1>
							<p class="wow fadeInUp" data-wow-duration="1s"
								data-wow-delay="1s"><%=Products.get(k).getInformation()%>
							</p>
							<a href="jsp/product_detail.jsp?gid=<%=Products.get(k).getId()%>"
								class="wow fadeInUp" data-wow-duration="1s"
								data-wow-delay="1.5s">shop now</a>
						</div>
					</div>
				</div>
			</div>
			...
			<div id="slider-caption-2" class="nivo-html-caption">
				<div class="container">
					<div class="row">
						<div class="hero-slider-content col-sm-8 col-xs-12">
							<h1 class="wow fadeInUp" data-wow-duration="1s"
								data-wow-delay="0.5s"><%=Products.get(j).getName()%></h1>
							<p class="wow fadeInUp" data-wow-duration="1s"
								data-wow-delay="1s"><%=Products.get(j).getInformation()%>
							</p>
							<a href="jsp/product_detail.jsp?gid=<%=Products.get(j).getId()%>"
								class="wow fadeInUp" data-wow-duration="1s"
								data-wow-delay="1.5s">shop now</a>
						</div>
					</div>
		...
		<div style="width: 80%; margin-left: 10%;" align="center">
			...
			%>
			<div id='<%=i%>'
				class="isotope-item chair home-decor col-lg-3 col-md-4 col-sm-6 col-xs-12 mb-50">
				<div class="product-item text-center">
					<!-- Product Image -->
					<div class="product-img">
						<!-- Image -->
						<a class="image"
							href="jsp/product_detail.jsp?gid=<%=goodsList.get(i).getId()%>">
							<img src="<%=goodsList.get(i).getPath()%>" alt="" />
						</a>
						<!-- Wishlist Button -->
						...
						<div class="action-btn fix">
							<a
								href="BuyProductServlet?pid=<%=goodsList.get(i).getId()%>&buyNumber=1"
								title="Buy"><i class="pe-7s-like"></i>购买</a> <a
								href="AddCartServlet?pid=<%=goodsList.get(i).getId()%>&buyNumber=1"
					...
					<div class="product-info text-left">
						...
						<h5 class="title">
							<a
								href="jsp/product_detail.jsp?gid=<%=goodsList.get(i).getId()%>"><%=goodsList.get(i).getName()%></a>
						</h5>
						<!-- Price Ratting -->
						<div class="price-ratting fix">
							<span class="price float-left"><span class="new">RMB
									<%=goodsList.get(i).getPrice()%></span></span> <span
		...
		<div class="row">
			<div class="text-center col-xs-12 mt-30">
				<button id="loadmore" class="btn load-more-product">load
					more</button>
			</div>
